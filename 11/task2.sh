...
function DoOutput(val) {
  ...
  if(next_output == 1) {
    ...
    # Update bounding box
    if(robo_pos_x < robo_min_x) {
      robo_min_x = robo_pos_x
    }
    if(robo_pos_x > robo_max_x) {
      robo_max_x = robo_pos_x
    }
    if(robo_pos_y < robo_min_y) {
      robo_min_y = robo_pos_y
    }
    if(robo_pos_y > robo_max_y) {
      robo_max_y = robo_pos_y
    }
...

{
  ...
  TURN_LEFT = 0
  TURN_RIGHT = 1 # LOL .. this is not right :D
  ...
  for(y = robo_max_y; y>=robo_min_y; y--) {
    for(x = robo_max_x; x>=robo_min_x; x--) {
      if(length(painting[x, y]) == 0) {
        printf " "
      } else if(painting[x, y] == WHITE) {
        printf "O"
      } else if(painting[x, y] == BLACK) {
        printf " "
      } else {
        printf "X"
      }
