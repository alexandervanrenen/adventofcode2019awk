...
function PrintArea() {
  ...
  print "Max distance from oxygen: " max_distance
}
...
#  print steps " -> " RobotReturnToStirng(val)
  ...
  # Only works if not a wall
  if(next_command > 0) {
    max_distance = distance_to_prev_location + 1 > max_distance ? distance_to_prev_location + 1 : max_distance
  }
  ...
  if(val == HAS_MOVED) {
    ...
    # Found oxygen, reset everything and restart search from here to get a good distance map
    delete area
    area[0, 0] = "O"
